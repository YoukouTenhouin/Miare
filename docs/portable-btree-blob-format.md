...
The backend publishes immutable copy-on-write generations. Each committed generation identifies one authenticated root set covering the ordered keyspace, Blob catalog, and allocation state. A transaction writes replacement structures without modifying any Authenticated extent reachable from a committed generation, then publishes its root set atomically. Superseded extents remain available while a live snapshot can observe their generation and become reclaimable afterward.
...
Authenticated extents refer directly to physical file ranges; v1 has no logical-page location map. Relocation therefore copies affected extents and rewrites their authenticated ancestor paths.

All backend-owned extents are self-framing, including B+ tree pages, overflow Values, Blob manifests, Blob chunks, and allocator structures. A child must agree with the independently authenticated location, bounds, generation, and role expected by its parent before any plaintext is released.

The format protects application keys, Values, Blob content, and allocator content, but does not conceal Structural metadata or provide deniability. Visible authenticated preambles expose unit roles and key domains, physical positions and spans, encoded and decoded sizes, compression representation, creation generations and logical sequences, Blob ownership, and nonces. Consequently an offline observer can infer physical structure, approximate logical sizes, compression ratios, update generations, and which physical units belong to the same Blob.
...
| `0x1000`–`0x1fff` | 4 KiB encrypted publication slot A |
| `0x2000`–`0x2fff` | 4 KiB encrypted publication slot B |
...
| 16 | 4 | Ciphertext length, `4016` |
| 20 | 4 | Flags, zero |
| 24 | 24 | Fresh XChaCha20-Poly1305-IETF nonce |
| 48 | 16 | Reserved, zero |
| 64 | 4016 | Ciphertext |
| 4080 | 16 | Authentication tag |

The slot uses the derived header key. Its associated data is exactly 4,176 bytes: the 16 bytes consisting of ASCII `MiareHeaderV1` followed by three zero bytes, the complete 4,096-byte visible bootstrap, and the complete 64-byte slot envelope. The slot index must match its physical offset. A structurally invalid or unauthenticated slot is ignored when the other slot proves a committed generation; if neither slot authenticates, open returns `AuthenticationFailed`.

The authenticated slot plaintext is exactly 4,016 bytes:
...
Generation starts at one with predecessor zero. Every later transaction or maintenance root publication increments generation by exactly one, records the prior generation as predecessor, and must not wrap. Initial creation writes both slots at generation one with identical semantic fields, independent nonces, and matching slot indices. Null root references represent empty structures.

For every generation `g` at least two, the designated slot index is `g mod 2`: even generations use slot A/index zero and odd generations use slot B/index one. A retry of an unpublished generation rewrites the same designated slot with a fresh nonce. Its other slot contains the authoritative predecessor when persistence begins.

The committed byte boundary is `high_water_blocks * allocationQuantumBytes`; reachable extents end within it. Authenticated slots normally hold equal initial generations or adjacent generations with a matching predecessor chain. Equal-generation slots must have identical semantic fields other than slot index. The valid newer generation wins. Contradictory authenticated slots are corruption; rollback of the complete file remains outside the threat model. Unknown required feature bits produce `UnsupportedFeature`; optional bits may be ignored only when they affect neither decoding nor semantics.

Fallback to the predecessor is permitted only when the designated newer slot is structurally invalid, torn, or unauthenticated. Once the newest slot authenticates, it establishes the committed generation: its canonical plaintext and all non-null roots receive framing, authentication, role, generation, and shallow root-header validation during open. Failure beneath that publication is `Corrupt` or the applicable provider/resource error and never causes silent predecessor selection.

## Capacity-profile format parameters

`allocationQuantumBytes` is a power of two from 512 bytes through 64 KiB inclusive. `DefaultLimits` uses 4 KiB. Extent positions and allocation spans are expressed in whole allocation quanta. The value is persisted numerically in the authenticated capacity profile and must match the opening implementation exactly.
...
| Encryption suite | 1 | XChaCha20-Poly1305-IETF |
| KDF algorithm | 1 | Miare BLAKE2b derivation v1 |
...
Common format, backend format, envelope, publication, page, manifest, allocator, capacity-profile, and codec-profile versions all begin at one. Unknown identifiers are never inferred from surrounding structure.
...
The containing authenticated structure supplies the expected unit role. `creation_generation` is nonzero and no greater than the containing structure's generation. Reference arithmetic must not overflow or address outside the file, overlap a protected common region, or violate the authenticated capacity profile. Distinct reachable live extents do not overlap; an overlap is valid only where references are byte-for-byte identical. The child preamble and associated data must agree with the reference and expected role before plaintext is released.

## Authenticated-extent framing

Every backend Authenticated extent begins with this exact 160-byte visible preamble:
...
| 104 | 24 | XChaCha20-Poly1305-IETF nonce |
| 128 | 32 | Reserved, zero |

The preamble is followed immediately by `stored_payload_length` ciphertext bytes, the full 16-byte authentication tag, and zero allocation padding. Consequently `encoded_length` is exactly `160 + stored_payload_length + 16`. The fixed preamble is parsed and bounded before a provider call but is not trusted until authentication succeeds.
...
All integer fields use canonical little-endian encoding. The profile digest covers the later-specified canonical serialization of every capacity-profile field. Parent bytes are not appended: authentication of the parent establishes the expected reference and role, while this fixed associated data independently binds the child's database, formats, suite, profile, physical identity, generation, role, owner, sequence, lengths, representation, and nonce.

### Nonce and key-usage limits
...
Authentication failure releases no plaintext. Failure while establishing the encrypted bootstrap is the intentionally conflated `AuthenticationFailed` outcome; after one publication authenticates and establishes database identity, failure of a reachable protected unit is `Corrupt` and enters recovery-required state.
...
Authentication completes before decompression. The decoder then requires one standard frame, the exact authenticated decoded content size, a window no larger than 16 MiB, no dictionary identifier or checksum, no trailing input, and exactly the expected bounded output before parsing or exposing plaintext. B+ tree pages of every role, overflow Values, and Blob chunks are eligible. Blob manifests and fixed allocator roots are not.
...
Leaf pages have no persistent sibling links. Forward and reverse cursors retain their authenticated root-to-leaf paths and cross leaf boundaries by ascending to an adjacent subtree and descending to its edge.
...
The visible extent unit kind identifies both tree role and leaf/internal type, and the encrypted page header repeats that identity. A per-Blob chunk-index page also places its Blob identifier in the extent preamble's owner identity. Role disagreement at any layer is corruption.
...
An inline Value resides completely in its ordered-keyspace leaf entry. A Value larger than `maxInlineValueBytes` resides in one independently authenticated overflow-value extent, optionally compressed as a single unit; its leaf entry stores the logical length and direct Extent reference. Blob storage, rather than overflow Values, provides chunked incremental access.

An overflow-value extent's decoded payload is exactly the raw Value bytes without an inner header. It uses the main-data key domain, zero owner identity and logical sequence, and a decoded length equal to the authenticated leaf length, greater than `maxInlineValueBytes`, and no greater than `maxValueBytes`. Compression, when selected, covers the complete raw Value. Authentication and bounded decompression finish before any Value bytes are returned.
...
V1 stores no whole-Blob checksum. Manifest authentication binds identity, version, length, chunking, and index root; each chunk independently authenticates its owner, ordinal, representation, and bytes.
...
A Blob-chunk extent uses the Blob key domain, exposes the same owner identifier, records its zero-based ordinal as logical sequence, and has the manifest content generation as creation generation. Its decoded payload is exactly the raw Blob bytes without another inner header. Every non-final chunk has `blobChunkBytes` decoded bytes; the final length is the positive remainder implied by the manifest. Compression, when selected, covers the complete raw chunk. The index key and reference, preamble owner, ordinal and generation, and manifest-derived expected length must all agree before content is exposed.
...
The uncompressed main-data-keyed allocator-root plaintext is exactly 160 bytes:
...
Counts include complete allocated spans and padding. The three authenticated counters partition every backend block below the high-water mark into reachable live, free, or retired state. They accelerate diagnostics but must equal a full verification traversal.
...
A write transaction owns a candidate allocator derived from the committed root; staging never changes committed allocation metadata. Reused runs are removed and appended tail blocks are added only in that candidate. Aborting a Blob writer or making staged structures unreachable returns their runs to the transaction-local free set, where they may be reused with fresh nonces.

Rollback or failure before publication discards the candidate: the committed root still classifies reused ranges as free, and appended bytes remain outside its high-water mark. Successful publication classifies every backend block below the new high-water mark as reachable, free, or retired. V1 has no persistent orphan catalog. After an uncertain publication, authenticated slot selection determines which complete allocator state is authoritative.
...
An unknown storage backend, encryption suite, or required feature is `UnsupportedFeature`. An unknown KDF, derivation, or codec profile, or any capacity-profile mismatch, is `IncompatibleProfile`. A known required capability whose provider is absent is `ProviderUnavailable`. Unknown optional bits may be ignored only when their definition affects neither parsing nor semantics, and an implementation that does not understand them preserves them in every later publication.

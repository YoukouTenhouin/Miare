...
`ProviderSet` exposes these construction functions according to compiled
capabilities:

```cpp
static ProviderSet none();
static ProviderSet systemCrypto();       // when libsodium support is enabled
static ProviderSet systemCompression();  // when Zstandard support is enabled
static ProviderSet system();             // when both are enabled
```
...
`std::hash<miare::BlobId>` is provided. Blob identifiers are generated by the
format-neutral system entropy source and collision-checked in the write
transaction's view. This entropy capability remains available without a crypto
provider and is independently injectable in tests. An aborted identifier is
never deliberately reused. An identifier carries no database identity,
generation, timestamp, or global uniqueness promise; using an identifier from
another database simply observes absence.
...
- keyed `open` returns `AuthenticationFailed` when a suite-1 publication cannot authenticate.
- a keyless entry point presented with suite 1 throws `KeyRequired`; a keyed
  entry point presented with suite 0 throws `UnexpectedKey`.
- keyed creation with `CreateOptions::encryptionSuite ==
  EncryptionSuite::None` throws `UnexpectedKey` before creating a temporary or
  target file; suite 0 can be created only by `createUnencrypted`.
...
public:
    ...
    static Database createUnencrypted(
        const std::filesystem::path& path,
        UnencryptedCreateOptions options = {},
        ProviderSet providers = ProviderSet::none(),
        Allocator allocator = {});

    static Database openUnencrypted(
        const std::filesystem::path& path,
        ProviderSet providers = ProviderSet::none(),
        OpenOptions options = {},
        Allocator allocator = {});

    static VerificationReport verifyUnencryptedFile(
        const std::filesystem::path& path,
        ProviderSet providers = ProviderSet::none(),
        Allocator allocator = {});
};
...
`ProviderSet` is an owned, type-erased value moved into the database session.
It may own cryptography, compression, both capabilities, or neither. Persisted
files identify algorithms and profiles, never provider implementations or
versions. A capability is required only after the selected suite or codec
establishes that it is needed; a missing required capability throws
`DatabaseError{Errc::ProviderUnavailable}`. `ProviderSet::none()` owns neither
optional capability. System provider constructors request crypto and
compression independently. There is no silent process-global provider
registry.

`CreateOptions` contains persisted choices for keyed creation and retains its
existing defaults. `UnencryptedCreateOptions` has no key or encryption-suite
field and defaults compression to `None`. `OpenOptions` contains runtime
budgets only. Each open entry point discovers backend, compression, encryption,
format, features, and capacity profile from the protected file, then requires
that its keyed or keyless API shape agree with the visible suite identifier.

V1 uses:

- `StorageBackend::BTree`, fixed for the database's lifetime;
- independently selected `Compression::None` or `Compression::ZStd`;
- `EncryptionSuite::None` through the keyless API or libsodium-compatible
  `EncryptionSuite::XChaCha20Poly1305Ietf` through the keyed API;
- for suite 1, a suite-selected 32-byte derived key, fresh random 24-byte nonce per independently authenticated unit, and full 16-byte tag;
...
Suite 0 accepts no key material. `createUnencrypted`, `openUnencrypted`, and
`verifyUnencryptedFile` have no key parameter and never manufacture or derive a
key internally. They never invoke AEAD encryption or decryption. Application
keys, Values, Blob bytes, and allocator bytes are plaintext. Protected units
carry an unkeyed corruption checksum and canonical framing; because the
checksum is publicly recomputable, suite 0 provides no confidentiality,
cryptographic authentication, origin guarantee, or adversarial tamper
resistance.
...
Failure to authenticate the encrypted publication slots returns
`AuthenticationFailed`, intentionally conflating an incorrect key with slot
tampering or corruption. No plaintext or finer reason is exposed. After an
encrypted publication establishes database identity and format, failure to
authenticate a protected unit is corruption and throws
`DatabaseError{Errc::Corrupt}`. Suite-0 publication or extent checksum failure
also throws or reports corruption, but is never described as authentication
failure.
...
Before persistence begins, public operations provide a strong logical exception guarantee: `ContractError`, `std::bad_alloc`, `ProviderUnavailable`, `ResourceLimit`, and ordinary I/O failure preserve public handle states, cursor position, and transaction contents. A failed mutable-buffer read may leave unspecified bytes in the caller buffer, but never releases bytes from a protected unit whose authentication or checksum has failed.
...
`verify` performs one full verification level: it applies the selected protection check and structurally validates every reachable unit, ordered-keyspace invariant, Blob manifest and chunk chain, and relevant recovery metadata. `VerificationReport` owns a validity result, counts and bytes checked, a fixed bounded number of physical findings, and a truncation flag; it never contains application keys, values, Blob content, or encryption material. Online confirmed corruption returns the report and enters `RecoveryRequired`. Offline verification has no session to affect. Encrypted publication rejection from keyed `verifyFile` returns `AuthenticationFailed`; keyless verification reports checksum corruption without an authentication claim.

...
Protect persistent content with independently authenticated libsodium-compatible XChaCha20-Poly1305-IETF units, fresh random 24-byte nonces, full 16-byte tags, and purpose-separated derived keys. The v1 B+ tree backend keeps all data-bearing runtime state in its main file and publishes immutable generations through dual authenticated slots. Scope the durability promise to validated local regular files whose storage stack honors the required barriers. The frozen recovery, maintenance, and verification contract fixes interruption outcomes, fail-closed recovery, operator workflows, and observable conformance gates over that representation.
...
- B+ tree updates, page and extent bytes, free and retired indexes, overflow Values, Blob physical layout, compression boundaries, authenticated-open corruption boundaries, interruption behavior, and maintenance algorithms are frozen. Cache implementation remains private; repair, salvage, and degraded reads are explicitly excluded from v1.
...
- All fourteen mapped specification work items covering the public API, transaction contract, durability, backend architecture, portable format, encryption, compression, Blob storage, capacity, recovery, maintenance, and verification are resolved or explicitly superseded. Their frozen contracts and ADRs authorize the sequenced implementation tickets without reopening product decisions implicitly.

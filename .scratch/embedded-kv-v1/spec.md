...
Protect the main file and every data-bearing runtime sidecar with independently authenticated libsodium-compatible XChaCha20-Poly1305-IETF units, fresh random 24-byte nonces, full 16-byte tags, and purpose-separated derived keys. Scope the durability promise to validated local regular files whose storage stack honors the required barriers. After a clean close, all committed state must be contained in one portable database file. Complete the remaining storage specification by fixing the B+ tree and Blob layouts, compatibility and recovery formats, maintenance algorithms, and observable conformance suite before implementing those layers.
...
- The common and backend-owned file regions must use an explicit cross-platform byte representation and identify format version, storage backend, feature set, capacity profile, integrity boundaries, roots, and recovery state. Open never performs an implicit upgrade; physical backward-readability and unsupported-version rules remain format work.
- The B+ tree update/recovery strategy, page and extent formats, free-space and overflow layouts, Blob physical layout, cache implementation, corruption boundaries, salvage policy, and maintenance algorithms remain specification work. They are not left to implementation discretion.
...
- The public contract fixes correctness limits and the representative desktop workload; later qualification must add measurable latency, throughput, memory, and amplification targets before architectural and format choices are accepted.
